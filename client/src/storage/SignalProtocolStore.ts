...
/**
 * In-memory implementation of the Signal Protocol `StorageType` interface.
 * Holds identity/prekey/session material for exactly one local user.
 *
 * SECURITY NOTE: this is a reference implementation for development. It
 * keeps everything in a JS Map with no persistence, so sessions do not
 * survive a page reload. For production, swap `persist`/`hydrate` for an
 * IndexedDB-backed store (never localStorage — it's trivially readable by
 * any XSS) and never transmit anything held here to the server; only the
 * public bundle built in `signalClient.ts` should ever leave the device.
 */
export class SignalProtocolStore implements StorageType {
  ...
  async put(key: string, value: unknown): Promise<void> {
    if (value === undefined) {
      throw new Error(`SignalProtocolStore.put: value for "${key}" is undefined`);
    }
  ...
  /** Returns true if this identifier's identity key CHANGED — callers should
   *  surface a safety-number warning to the user in that case. */
  ...
  async removeAllSessions(identifier: string): Promise<void> {
    for (const key of Array.from(this.data.keys())) {
      if (key.startsWith(`session${identifier}`)) this.data.delete(key);
    }
  }
}
